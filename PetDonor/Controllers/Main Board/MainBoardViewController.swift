...
class MainBoardViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    let (right, left) = swipeGesturesConfigure(target: self,
                                               rightAction: #selector(rightSwipeAction(_:)),
                                               leftAction: #selector(leftSwipeAction(_:)))
  ...
  private func reloadPetList(filter: [String: Any]) {
    ...
    Task {
      do {
        let snapshot = try await database.getPetsWithFilter(filter: filter)
        pets = database.convertSnapshotToPet(snapshot: snapshot)
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == toPetCardSegueIdentifier,
       let destinationVC = segue.destination as? PetCardViewController,
       let pet = pet {
      destinationVC.pet = pet
    } else if segue.identifier == toFiltersListSegueIdentifier,
              let destinationVC = segue.destination as? FiltersViewController {
      destinationVC.filter = filter
      destinationVC.delegate = self
    }
  }
  private func swipeGesturesConfigure(target: Any?,
                                      rightAction: Selector?,
                                      leftAction: Selector?) ->
  (rightGesture: UISwipeGestureRecognizer, leftGesture: UISwipeGestureRecognizer) {
...
extension MainBoardViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if let ref = pet.imageUrl, ref.count > 0 {
      ...
      guard let cell = tableView.dequeueReusableCell(withIdentifier: BoardTextOnlyTableViewCell.identifier,
                                                     for: indexPath) as? BoardTextOnlyTableViewCell
      else {
        return UITableViewCell()
      }
      cell.petTypeLabel.text = pet.petType?.rawValue
  ...
  func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
    ...
    if pets.count >= 7, indexPath.row == pets.count - 2 {
      ...
      Task {
        do {
          let snapshot = try await database.getNextPetsPart(from: snapshot, filterOrNil: filter)
          let petsArray = database.convertSnapshotToPet(snapshot: snapshot)
